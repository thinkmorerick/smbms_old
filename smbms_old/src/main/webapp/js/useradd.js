...
$(function(){
	...
	userCode.bind("blur",function(){
		...
		$.ajax({
			...
			error:function(data){ //当访问时候，404，500等非200的错误状态码
	...
	userPassword.bind("focus",function(){
		validateTip(userPassword.next(),{"color":"#666666"},"* 密码长度必须是大于6小于20的字符",false);
	}).bind("blur",function(){
		if(userPassword.val() != null && userPassword.val().length >6 && userPassword.val().length < 20){
			...
			validateTip(userPassword.next(),{"color":"red"},imgNo+" 密码不符合规范，请重新输入",false);
		}
	});
	...
	ruserPassword.bind("focus",function(){
		...
		if(userPassword.val() != null && ruserPassword.val().length >6 && ruserPassword.val().length < 20 && userPassword.val()==ruserPassword.val()){
